...
public class Lab5 extends GLJPanel implements GLEventListener, KeyListener{
	 ...
	 */
	private void piramidka(int n, GL2 gl2)
	{
		float x,y;
		var theta = 2*Math.PI / n; //stopień potrzebny do obliczenia współrzędnych punktu na okręgu
		...
		for (int i = 1; i <= n+1; i++)
		{
			...
			x = (float)Math.cos(theta * i); //współrzecna x
			y = (float)Math.sin(theta * i); //współrzędna y
			gl2.glVertex3f(x, y, 4);
		}
		gl2.glEnd();
	...
	private void korkociong(int n, GL2 gl2)
	{
		gl2.glColor3f(0,1,0);
		gl2.glRotatef(270,1,0,0);
		int ilosc = 40; //ilosc kwadracików przypadająca na jeden okrąg
		var theta = 2*Math.PI / ilosc; //stopień potrzebny do obliczenia współrzędnych punktu na okręgu
		float x,y;

		for(float i=1;i<=n*ilosc;i++)
		{
			float wielkoscPunktu = (float)0.07 * i;
			gl2.glPointSize(wielkoscPunktu); //wielkość punktu
			gl2.glBegin(GL.GL_POINTS);
			x= (float)Math.cos(theta * i); //współrzecna x
			y= (float)Math.sin(theta * i); //współrzędna y
			gl2.glVertex3f(x*(0.01f*i), y*(0.01f*i), (i/ilosc)-(n/2)); //przesunięcia współrzednych
			gl2.glEnd();
		}
