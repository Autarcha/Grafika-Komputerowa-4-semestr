...
public class Lab6 extends GLJPanel implements GLEventListener  {
	...
	public void display(GLAutoDrawable drawable) {
		...
		gl2.glTranslated(0,0,-3.5f);
		//ścianki
		gl2.glBegin(GL.GL_TRIANGLE_FAN);
		gl2.glVertex3f(0,0,0);
		gl2.glNormal3f(0,0, 4);
		for (int i = 1; i <= n+1; i++)
		{
			if (i < 12)
			gl2.glMaterialfv(gl2.GL_FRONT_AND_BACK, GL_AMBIENT_AND_DIFFUSE, FloatBuffer.wrap(materials[i]));
			else
				gl2.glMaterialfv(gl2.GL_FRONT_AND_BACK, GL_AMBIENT_AND_DIFFUSE, FloatBuffer.wrap(materials[i-10]));
			x = (float)Math.cos(theta * i); //współrzecna x
			y = (float)Math.sin(theta * i); //współrzędna y
			gl2.glVertex3f(x, y, 4);
			gl2.glNormal3f(x,y, 4);
		}
		gl2.glEnd();
	...
	public void init(GLAutoDrawable drawable) {
		...
		gl2.glLightfv(GL2.GL_LIGHT2, GL2.GL_AMBIENT, FloatBuffer.wrap(new float[] {12f,1f,0f,0.3f}));
		gl2.glLightfv(GL2.GL_LIGHT2, GL2.GL_POSITION, FloatBuffer.wrap(new float[] {-5f,-0.5f,0f,0.3f}));
